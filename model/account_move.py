...
class AccountMove(models.Model):
    ...
    einv_sa_seg_delivery_date = fields.Date(string='Delivery Date', default=fields.Date.context_today, copy=False)
    einv_sa_seg_show_delivery_date = fields.Boolean(compute='_compute_einv_show_delivery_date')
    einv_sa_seg_qr_code_str = fields.Char(string='Zatka QR Code', compute='_compute_eniv_qr_code_str')
    ...
    def _compute_einv_sa_seg_confirmation_datetime(self):
        for move in self:
            move.einv_sa_seg_confirmed = False
            if not move.einv_sa_seg_confirmation_datetime and move.state == 'posted' \
                    and move.move_type in ('out_invoice', 'out_refund'):
                move.einv_sa_seg_confirmed = True
                move.einv_sa_seg_confirmation_datetime = move.create_date

    @api.depends('country_code', 'move_type')
    def _compute_einv_show_delivery_date(self):
        for move in self:
            # move.einv_sa_seg_show_delivery_date = False
            move.einv_sa_seg_show_delivery_date = move.country_code == 'SA' and move.move_type in (
                'out_invoice', 'out_refund')

    @api.depends('amount_total', 'amount_untaxed', 'einv_sa_seg_confirmation_datetime', 'company_id', 'company_id.vat')
    def _compute_eniv_qr_code_str(self):
        ...
        for record in self:
            qr_code_str = ''
            if record.einv_sa_seg_confirmation_datetime and record.company_id.vat:
                seller_name_enc = get_qr_encoding(1, record.company_id.display_name)
                company_vat_enc = get_qr_encoding(2, record.company_id.vat)
                time_sa = fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'),
                                                            record.einv_sa_seg_confirmation_datetime)
            ...
            record.einv_sa_seg_qr_code_str = qr_code_str

    def _post(self, soft=True):
        res = super()._post(soft)
        for record in self:
            if record.country_code == 'SA' and record.move_type in ('out_invoice', 'out_refund'):
                if not record.einv_sa_seg_show_delivery_date:
                    raise UserError(_('Delivery Date cannot be empty'))
                if record.einv_sa_seg_delivery_date < record.invoice_date:
                    raise UserError(_('Delivery Date cannot be before Invoice Date'))
                self.write({
                    'einv_sa_seg_confirmation_datetime': fields.Datetime.now()
                })
        return res
